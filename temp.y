...
%token RETURN 
%token VOIDTYPE 
%type<dtype> EXPR T G M 
%type<dtype> condition inner_condition 
%type<dtype> datatype assignment 
...
whileloop : WHILE OPENBRACKET condition CLOSEDBRACKET
                OPENBRACE enter_scope
                    code DOT
                        CLOSEDBRACE exit_scope;
...
function_header : datatype IDENTIFIER OPENBRACKET
{
...
list : EXPR
{
    ...
    else if (!type_compatible($1, current_param->type))
    {
        if (!param_error)
        {
            fprintf(stderr, "Line %d:Parameter %d of %s has type %s, expected %s\n", yylineno, arg_count, current_function->name, type_name($1), type_name(current_param->type));
            param_error = true;
        }
    }
    if (current_param)
...
| list COMMA EXPR
{
    ...
    else if (!type_compatible($3, current_param->type))
    {
        if (!param_error)
        {
            fprintf(stderr, "Line %d:Parameter %d of %s has type %s, expected %s\n", yylineno, arg_count, current_function->name, type_name($1), type_name(current_param->type));
...
switchstmt : SWITCH OPENBRACKET IDENTIFIER CLOSEDBRACKET
...
{ // net2aked en identifier of type int 3lshan manensash
    ...
    {
        fprintf(stderr, "Line %d: Variable %s must be integer", yylineno, $3);
    }
    else
    {
        s->used = true;
...
inner_case : CASE OPENBRACKET INTEGER CLOSEDBRACKET
                 OPENBRACE enter_scope
                     code DOT
                         CLOSEDBRACE exit_scope;
...
| EXPR EQUALITY BOOLEAN
{
    ...
    {
        fprintf(stderr, "Line %d:Invalid comparison operands. Cannot %s to boolean", yylineno, type_name($1));
        $$ = SYM_ERROR;
    }
}
| EXPR EQUALITY EXPR
...
{
    if ((($1 == SYM_INT || $1 == SYM_FLOAT) && ($3 == SYM_INT || $3 == SYM_FLOAT)) ||
        ($1 == SYM_BOOL && $3 == SYM_BOOL))
        $$ = SYM_BOOL;
    else
    {
        fprintf(stderr, "Line %d:Invalid comparison operands. Cannot compare %s to %s", yylineno, type_name($1), type_name($3));
...
assignment : IDENTIFIER EQUAL EXPR
{
    ...
    else
    {
        ...
        {
            fprintf(stderr, "Line %d:Cannot assign to const variable %s ", yylineno, $1);
        }
        else
        {
            s->initialized = true;
